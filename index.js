    ...
    let promises = files.map((file) =>
        (async () => {
            let output = run(
                ...
                await fs.readFile(
                    `problems/${message.problem_name}/testcases/in/${file}`
                ),
                time_limit
            );
            let correct_output = fs.readFile(
                `problems/${message.problem_name}/testcases/out/${file}`
            );
            let { stdout, time, killed, status, container_rm } = await output;
            container_rms.push(container_rm);
            correct_output = (await correct_output)
                .toString()
                .replace('\r\n', '\n')
                .replace(/(\s)+$/g, '');
            let stdout_output = stdout
                .toString()
                .replace('\r\n', '\n')
                .replace(/(\s)+$/g, '');
            let result = {
                ...
                result:
                    status !== 'Exited(0)'
                        ? 're'
                        : typeof stdout === 'string' && stdout_output === correct_output
                            ? 'ac'
                            : 'wa',
                time: time,
                killed
...
const handle_code_test = async (ws, message) => {
    ...
    const { stdout, stderr, time, killed, status, container_rm } = await run(
        lang,
        compile_result.commit_id,
...
const compile = async (lang, code) => {
    ...
    if (output === null) {
        ...
        let commit_id = (
            await execFile('docker', ['commit', container_id])
        ).stdout.slice(0, -1);
        result = {
            type: 'success',
            commit_id
